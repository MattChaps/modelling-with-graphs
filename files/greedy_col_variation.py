...
def find_next_vertex(G):
    ...
    neighbours = [] 
    for j in visited_counter:
        for i in list(G.neighbors(j)):
            if i not in visited_counter and i not in neighbours:
                neighbours.append(i)
...
def greedy(G):
    ...
    global visited_counter
    visited_counter = [1]
    m = len(visited_counter)
    vertex = 1
    c = find_smallest_color(G,vertex)
    G.node[vertex]['color'] = c
    while m < n:
        vertex = find_next_vertex(G)
        visited_counter.append(vertex)
        c = find_smallest_color(G,vertex)
        G.node[vertex]['color'] = c
        m = len(visited_counter)
